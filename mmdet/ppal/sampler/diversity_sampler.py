...
class DiversitySampler(BaseALSampler):
    ...
    def kmeans(dis_matrix, K, n_iter=100):
        ...
        for _ in range(n_iter):
            ...
            for i in range(K):
                ...
                # sys_echo('>>>> cluster number: %d, indices: %s' % (i, str(cluster_i)))
                ...
                if len(cluster_i) > 0:
                    # Compute distances within the current cluster
                    dis_mat_i = dis_matrix[cluster_i][:, cluster_i]
                    # Find the index of the data point minimizing the sum of distances
                    new_centroid_i = cluster_i[np.argmin(dis_mat_i.sum(axis=1))]
                    new_centroids.append(new_centroid_i)
    ...
    def al_acquisition(self, image_dis_path, last_label_path):
        with open(self.oracle_path) as f:
        ...
        for i in range(len(sampled_img_ids)):
            img_id = sampled_img_ids[i]
            image_name = str(img_id) + ".jpg"  # Convert img_id to string for concatenation
            image_real_id = None  # Initialize before the loop

            # Iterate through the "images" list
            for image_info in data_oracle['images']:
                if image_info['file_name'] == image_name:
                    image_real_id = image_info['id']
                    break  # Break the loop once a match is found

            # Replace img_id with image_real_id in sampled_img_ids
            sampled_img_ids[i] = image_real_id

            # You can do further processing using image_real_id if needed

            rest_image_ids.remove(image_real_id)


        unsampled_img_ids = rest_image_ids
        print(len(unsampled_img_ids))
        print(len(sampled_img_ids))
