...
class BaseALSampler(object):
    ...
    def create_jsons(self, sampled_img_ids, unsampled_img_ids, last_labeled_json, out_label_path, out_unlabeled_path):
        ...
        # Check for duplicates between last_labeled_img_ids and sampled_img_ids
        duplicate_ids = set(last_labeled_img_ids) & set(sampled_img_ids)
        ...
        sys_echo('--->>> Last round labeled set size: %d' % (len(last_labeled_img_ids)))
        sys_echo('--->>> all_labeled_img_ids set size: %d' % (len(set(all_labeled_img_ids))))

        sys_echo('--->>> unsampled_img_ids labeled set size: %d' % (len(unsampled_img_ids)))
        sys_echo('--->>> sampled_img_ids labeled set size: %d' % (len(sampled_img_ids)))
        sys_echo('--->>> image_pool_size labeled set size: %d' % self.image_pool_size)
